...
dates <- seq.Date(as.Date("2023-03-01"), as.Date("2023-09-01"), "1 month")
...
mta1 |> 
  filter(date %in% as.Date(c("2023-06-01", "2023-07-01", "2023-08-01")))
...
pdata <- pmtwage |> 
  mutate(type="history") |> 
  bind_rows(fcdat) |> 
  mutate(type=factor(type, levels=c("history", "forecast"))) |> 
...
pdata |> 
  mutate(year=year(date),
         period=ifelse(month(date) < 5, "jan_apr", "may_dec")) |> 
  summarise(value=sum(value), .by=c(year, period)) |> 
  filter(year <= 2024) |> 
  pivot_wider(names_from = period) |> 
  mutate(tot=jan_apr + may_dec) |> 
...
pdata |> 
  mutate(year=year(date),
         period=ifelse(month(date) < 9, "jan_aug", "sep_dec")) |> 
  summarise(value=sum(value), .by=c(year, period)) |> 
  filter(year <= 2024) |> 
  pivot_wider(names_from = period) |> 
  mutate(tot=jan_aug + sep_dec) |> 
  mutate(across(.cols=-year,
                ~ .x / lag(.x) - 1,
         .names = "{.col}_pch"))

fc |> 
  autoplot(pmtwage, level = NULL) +
  labs(x="") +
  theme_bw()
...
pmtwage2 |> 
  ...
  scale_y_continuous(name="$ millions", labels = label_comma(scale=1e-3)) +
  labs(x="") +
  ggtitle("Payroll mobility tax collections (wage tax only)") +
  theme_bw()
